...
public class HibernateUserLevelDao extends HibernateBaseDao<UserLevel>
    ...
    public UserLevel getNextLevel(final Long id) {
        ...
        if (userLevel != null) {
            nextUserLevel = getHibernateTemplate().execute(
                    new HibernateCallback<UserLevel>() {
                        public UserLevel doInHibernate(Session session) {
                            ...
                            Criteria criteria = session
                                    .createCriteria(UserLevel.class);
                            criteria.add(Restrictions.gt("levelScore",
                                    userLevel.getLevelScore()));
                            criteria.add(Restrictions.eq("levelStatus", true));
                            criteria.addOrder(Order.asc("levelScore"));

                            if (!criteria.list().isEmpty()) {
                                newUserLevel = (UserLevel) criteria.list()
                                        .iterator().next();
                            }
    ...
    public UserLevel getPreLevel(final Long id) {
        ...
        if (userLevel != null) {
            lastUserLevel = getHibernateTemplate().execute(
                    new HibernateCallback<UserLevel>() {
                        public UserLevel doInHibernate(Session session) {
                            UserLevel newUserLevel = null;
                            ...
                            Criteria criteria = session
                                    .createCriteria(UserLevel.class);
                            criteria.add(Restrictions.lt("levelScore",
                                    userLevel.getLevelScore()));
                            criteria.add(Restrictions.eq("levelStatus", true));
    ...
    public List<User> getUsers(final Long userLevelId, final int curPage,
            final int pageSize) {
        ...
        return getHibernateTemplate().execute(
                new HibernateCallback<List<User>>() {
                    ...
                    public List<User> doInHibernate(Session session) {
                        ...
                        criteria.add(Restrictions.eq("memberShip.level.id",
                                userLevelId));
                        ...
                        criteria.setProjection(
                                Projections
                                        ...
                                        .add(Property.forName("cellphone").as(
                                                "cellphone"))
                                        .add(Property.forName("memberShip").as(
                                                "memberShip"))
                                        .add(Property.forName("username").as(
                                                "username"))
                                        .add(Property.forName("userStatus").as(
                                                "userStatus")))
