...
public class RoleFunctionService extends BaseService {
    ...
    public List<RoleFunction> findAllRoleFunctions() {
        return roleFunctionDao.findAllRoleFunctions();
    }

    public RoleFunction findRoleFunctionByRoleFunctionName(
            String roleFunctionName) {
        return roleFunctionDao
                .findRoleFunctionByRoleFunctionName(roleFunctionName);
    }
    ...
    public List<Function> findFunctionsByRoleName(String roleName) {
        ...
        List<RoleFunction> roleFunctions = roleFunctionDao
                .findRoleFunctionsByRoleName(roleName);
        Function function = null;
        for (RoleFunction roleFunction : roleFunctions) {
            function = functionDao.findFunctionsByFunctionName(roleFunction
                    .getFunctionName());
    ...
    public List<RoleFunction> findRoleFunctionsByFunctionName(
            String functionName) {
        return roleFunctionDao.findRoleFunctionsByFunctionName(functionName);
    }
    ...
    public RoleFunction saveRoleFunction(Role role, Function function) {
        ...
        if (null != roleDB && null != functionDB) {
            String roleFunctionName = roleDB.getRoleName()
                    + functionDB.getFunctionName();
            String roleName = roleDB.getRoleName();
            String functionName = functionDB.getFunctionName();
            roleFunction = new RoleFunction(roleFunctionName, roleName,
                    functionName);
            RoleFunction roleFunctionDB = (RoleFunction) roleFunctionDao
                    .get(roleFunctionName);
            // 如果DB不存在，就添加
    ...
    public void removeRoleFunction(Role role, Function function) {
        ...
        // RoleFunction roleFunction = null;
        if (null != roleDB && null != functionDB) {
            String roleFunction = roleDB.getRoleName()
                    + functionDB.getFunctionName();
            // String roleName = roleDB.getRoleName();
            // String functionName = functionDB.getFunctionName();
            // roleFunction = new RoleFunction(roleFunction, roleName,
            // functionName);
            RoleFunction roleFunctionDB = (RoleFunction) roleFunctionDao
                    .get(roleFunction);
            // 如果DB不存在，就添加
    ...
    public void updateRoleFunction(String roleName, String myFunctions) {
        ...
        List<String> myFunctionNames = StringUtil.stringToList(myFunctions,
                Constants.REGEX);
