...
public class RoleFunctionDao extends BaseDao {
    ...
    public List<RoleFunction> findAllRoleFunctions() {
        return super.getHibernateTemplate().find("FROM RoleFunction");
    }
    ...
    public RoleFunction findRoleFunctionByRoleFunctionName(
        ...
        List<RoleFunction> list = super.getHibernateTemplate().find(
                "FROM RoleFunction f WHERE f.roleFunctionName=?",
                roleFunctionName);
    ...
    public List<RoleFunction> findRoleFunctionsByRoleName(String roleName) {
        List<RoleFunction> list = super.getHibernateTemplate().find(
                "FROM RoleFunction f WHERE f.roleName=?", roleName);
        return list;
    }
    ...
    public List<RoleFunction> findRoleFunctionsByFunctionName(
            String functionName) {
        List<RoleFunction> list = super.getHibernateTemplate().find(
                "FROM RoleFunction f WHERE f.functionName=?", functionName);
        return list;
    }
}