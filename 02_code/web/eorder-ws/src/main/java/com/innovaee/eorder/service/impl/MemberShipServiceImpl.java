...
public class MemberShipServiceImpl implements MemberShipServcie {
    ...
    /**
     * 添加新会员等级
     * @param userLevelVO - 会员等级信息
     * @return 新会员等级实体
     * @throws DuplicateNameException 会员等级命名重复异常
     */
    @Override    
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false, rollbackFor = Exception.class)
    public UserLevel addUserLevel(UserLevelVO userLevelVO)
            throws DuplicateNameException {
        UserLevel userLevel = userLevelDao.getUserLevelByName(userLevelVO.getName());
        //1. 检查是否有同名的用户等级存在
        if(null != userLevel) {
    ...
    public UserLevel updateUserLevel(UserLevelVO userLevelVO)
        ...
        if(null == userLevel) {
            throw new UserLevelNotFoundException(
                    MessageUtil.getMessage("user_level_id", ""+userLevelVO.getId()));
        } else {
    ...
    /**
     *  根据指定名字获取会员等级
     * @param userLevelName 会员等级名字
     * @return 会员等级实体
     * @throws UserLevelNotFoundException 会员等级未找到异常
     */
    @Override
    public UserLevel getUserLevelByName(String userLevelName)
        ...
        if(null == userLevel) {
            throw new UserLevelNotFoundException(
                    MessageUtil.getMessage("user_level_name", userLevelName));
        }
     ...
     * @param pageSize 分页大小
     * @return 当前分页内的会员等级信息
     * @throws PageIndexOutOfBoundExcpeiton 非法当前页异常
     * @throws InvalidPageSizeException 非法的分页大小异常
     */
    @Override
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false, rollbackFor = Exception.class)
    public MemberShip addMemberShipToUser(Long userLevelId, Long userId, boolean forceLinkage)
            throws UserNotFoundException, UserLevelNotFoundException, MeberShipAlreadyExistException {
        //1. 检查要添加会员信息的用户是否存在
        User user = userDao.get(userId);
        if(null == user) {
            throw new UserNotFoundException(
                    MessageUtil.getMessage("user_id", ""+userId));
        }
        //2. 检查要添加到会员等级是否存在
        UserLevel userLevel = userLevelDao.get(userLevelId);
        if(null == userLevel) {
        ...
        if(user.getMemberShip() != null) {
            ...
            if(forceLinkage) {
                ...
                throw new MeberShipAlreadyExistException(user.getMemberShip().getId());
            }
        }
        //4. 创建新的会员信息复制给用户
    ...
    public MemberShip updateUserLevelOfUser(Long userId, int downOrUp)
        ...
        if(null == memberShip ){
            throw new UpdateUserLevelException(MessageUtil.getMessage("no_member_ship"));
        }
        
        UserLevel userLevel = null;
        
        if(1 == downOrUp) {
            userLevel = userLevelDao.getNextLevel(memberShip.getLevel().getId());
        } else {
            userLevel = userLevelDao.getLastLevel(memberShip.getLevel().getId());
        }
        //不能升级/降级的时候抛出异常
        if(null == userLevel) {
            throw new UpdateUserLevelException(
                    MessageUtil.getMessage("up_down_user_level", 
                            ""+downOrUp, 
                           memberShip.getLevel().getLevelName()));
        }
